...
#
# This kit uses a three-layer context system (AGENTS.md + skills + KB). The
...
# skill, or AGENTS.md) and deletes them before continuing.
#
# HARNESS SCOPE: this hook is Claude Code-specific — it checks Claude
# Code's auto-memory path at ~/.claude/projects/<slug>/memory/. If your
# harness has a different memory mechanism, either port this hook to
# check that path or disable it. The three-layer discipline still applies
# universally; only the path check is Claude Code-specific.
#
...
This repo does not use auto memory. Translate each file into the right layer and delete it:
  - Ethos / collaboration rule    → AGENTS.md
